...
class ProjectController {
    ...
    async CreateProject(req, res, next) {
        try {
            ...
            return res.status(201).json({
                status: 201, 
                success: true,
                message: `${title} created successfully. ✨🎉 Owner: ${req.user.username}`
            })
        } catch (error) {
            next(error);
        }
    ...
    async UpdateProjectProfile(req, res, next) {
        try {
            ...
            if (result.modifiedCount > 0) return res.status(200).json({ status: 200, success: true, message: "Profile Was Updated Successfully ✨🎉", profile: `${req.protocol}://${req.get("host")}${image}` })
